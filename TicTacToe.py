    ...
    for i in range(3):
        for j in range(3):
            if items[i][j] == "0":
                ...
                pygame.draw.circle(scr, (0, 0, 255), (j * 100 + 5, i * 100 +5), (j * 100 + 95, i * 100 + 95), 3)
                pygame.draw.circle(scr, (0, 0, 255), (j * 100 + 5, i * 100 +5), (j * 100 + 95, i * 100 + 95), 3)  
...
while mainloop:
    for event in pygame.event.get():
        ...
        if event.type == pegame.MOUSEBUTTONDOWN and not game_over:
            ...
            if player_win or ai_win:
                game_over = True
                if player_win:
                    pygame.display.set_caption("Ви перемогли!")
                else:
                    pegame.display.set_caption("Комп'ютер переміг!")
            elif field[0].count("x") + field[0].count("0") + field[1].count("x") + \ 
                    field[1].count("0") + field[2].count("x") + field[2].count("0") == 8:
                        game_over = True
                        
                pygame.display.set_caption("Нічия!")
            
            draw_tic_tac_toe(screen, field )
    
    draw_grid(screen)
    window.blit(scree, (0, 0))
